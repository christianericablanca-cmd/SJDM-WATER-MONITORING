...
function MapInner({ reports, businesses, reportIconCache, filteredReports, showBusinesses, businessesWithCoords }: {
  reports: WaterReport[];
  businesses: Business[];
            ...
            <Popup>
              <div className="space-y-1 min-w-[170px] max-w-[220px]">
                ...
                <div className="text-xs sm:text-sm">
                  {ISSUE_EMOJI[report.issue_type]} {ISSUE_TYPES.find(t => t.value === report.issue_type)?.label}
                </div>
                {report.water_provider && (
                  <div className="text-[9px] sm:text-[10px] text-muted-foreground/75">
    ...
    <div className="space-y-2 sm:space-y-3" ref={wrapperRef}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm min-w-0">
          ...
          <span className="text-muted-foreground hidden sm:inline">of {reports.length} reports</span>
          <span className="text-muted-foreground text-[10px] sm:text-xs truncate">
            · {new Set(filteredReports.map((r) => r.barangay)).size} barangays
          </span>
        </div>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={providerFilter} onValueChange={setProviderFilter}>
            <SelectTrigger className="w-full sm:w-[190px] h-11 sm:h-9 text-sm">
              <SelectValue placeholder="Water provider" />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex overflow-x-auto gap-2 sm:gap-2.5 pb-1 text-[10px] sm:text-xs text-muted-foreground -mx-1 px-1 flex-1">
          {ISSUE_TYPES.map((t) => (
            <div key={t.value} className="flex items-center gap-1 sm:gap-1.5 whitespace-nowrap shrink-0">
              <span className="inline-block w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: MARKER_COLORS[t.value] }} />
              {t.label}
            </div>
          ))}
        </div>
      ...
      {/* Map */}
      <div className="h-[55vh] min-h-[350px] sm:h-[600px] rounded-xl border overflow-hidden shadow-card relative bg-muted isolate">
        {mounted ? (
          <MapInner
            reports={reports}
